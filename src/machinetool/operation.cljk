(ns machinetool.operation
  "OperationActor -- one machine-tool-manufacturing operation = one
  supervised actor run, expressed as a langgraph-clj StateGraph. The
  advisor (Machine Tool Advisor) is sealed into a single node (:advise);
  its proposal is ALWAYS routed through the Machine Tool Governor
  ...
  One graph run = one machine-tool-manufacturing operation (intake ->
  ...
  decision to a human operator (the machine-tool manufacturing
  engineer). The approver resumes with `{:approval {:status :approved}}`
  (or :rejected). `:actuation/dispatch-unit`/`:actuation/issue-
  accuracy-certificate` ALWAYS reach this node when the governor is
  clean -- see `machinetool.phase`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [machinetool.machinetooladvisor :as machinetooladvisor]
...
(defn build
  ...
  `machinetool.store/Store`).
  opts:
    :advisor      -- a `machinetool.machinetooladvisor/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (machinetooladvisor/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; Machine Tool Advisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (machinetooladvisor/-advise advisor store request)]
            {:proposal p :audit [(machinetooladvisor/trace request p)]})))

      ;; Machine Tool Governor -- independent censor (separate system than the LLM).
      (g/add-node :govern
